...
public class BIRT_ActionListener_TestScript_Create implements BIRT_ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (null != e)
		{
			try
			{
				if (e.getActionCommand().equals(BIRT_ActionListener.APP_WINDOWS_PANELS_TESTSCRIPT_CREATE_CANCEL))
				{
				...
				else if (e.getActionCommand().equals(BIRT_ActionListener.APP_WINDOWS_PANELS_TESTSCRIPT_CREATE_SAVE))
				{
					BIRT_Panel_TestScript_Create objBiPanel_TestScript_Create = BIRT_Panels.getPanelBIRT_TestScript_Create();
					if (objBiPanel_TestScript_Create.ensureFieldsProper())
					{
						if (BIRT_Persistence.getObjBIPersistence_TestScript().isTestScriptNameDuplicate(objBiPanel_TestScript_Create.getTestScriptName()))
						{
						...
						else
						{
							...
							if (BIRT_Persistence.getObjBIPersistence_TestScript().saveTestScriptNTestSteps(BIRT_DataObject.getIProjectID(),
									BIRT_Panels.getPanelBIRT_TestScript_Create().getTestScriptName(), BIRT_Panels.getPanelBIRT_TestScript_Create().getTestScriptDescription(),
									BIRT_DataObject.getTestScript_CreateTestSteps()))
								;
							{
				...
				else if ((e.getActionCommand().equals(BIRT_ActionListener.APP_WINDOWS_PANELS_TESTSCRIPT_CREATE_CREATE_STEP)))
				{
				...
				else if ((e.getActionCommand().equals(BIRT_ActionListener.APP_WINDOWS_PANELS_TESTSTEP_CREATE_CREATESTEP)))
				{
					if (BIRT_Panels.getPanelBIRT_TestStep_Create().ensureFieldsProper())
					{
						...
						BIRT_Panels.getPanelBIRT_TestStep_Create().resetContents();
						BIRT_Panels.getPanelBIRT_TestScript_Create().updateContents();
						BIRT_Windows.getObjMainWindow().resetNAdd(BIRT_Panels.getPanelBIRT_TestScript_Create());
					}
				}
				else if ((e.getActionCommand().equals(BIRT_ActionListener.APP_WINDOWS_PANELS_TESTSCRIPT_CREATE_IMPORT)))
				{
				...
				else if ((e.getActionCommand().equals(BIRT_ActionListener.APP_WINDOWS_PANELS_TESTSCRIPT_CREATE_EXPORT)))
				{
					FileDialog objFileDialog = new FileDialog();
					...
					if (iSelectedState == JFileChooser.APPROVE_OPTION)
					{
						...
						try
						{
							...
							for (int iCtr = 0; iCtr < objArraySize; iCtr++)
							{
								...
								bw.write("\n");
								//System.out.println("Hi, I am writing...");
							}
